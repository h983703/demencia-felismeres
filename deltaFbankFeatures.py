...
korrelacio = open(basefolder + 'txt/fbank/deltaprobaKorrelacio.txt', 'w')
vektor = open(basefolder + 'txt/fbank/deltaprobaFeature.txt', 'w')
sablon_name = 'CSVDFBANK/B_feature_'
kiterjesztes='.csv'
cnt = 0
lepeskoz = [3, 5, 10, 20]
...
for lepes in lepeskoz:
    ...
    for z in dir_list:
        ...
        (temp, energy) = spfeatures.fbank(wavdata, samplerate=rate, nfilt=40)
        fbank_feat = np.column_stack((fbank_feat, np.log(energy)))
        fbank_d_feat = spfeatures.delta(fbank_feat, 2)
        #fbank_dd_feat = spfeatures.delta(fbank_d_feat, 2)

        # full_feat = fbank_feat

        full_feat = np.concatenate((fbank_feat, fbank_d_feat), axis=1)
        #full_feat = np.concatenate((fbank_feat, fbank_d_feat, fbank_dd_feat), axis=1)
        ...
        print(A.shape)
        B = np.reshape(A, (6724, 1))
        #B = np.reshape(A, (15129, 1))
        print(B.shape)
        B.tofile(name, sep=',')
        cnt=cnt+1
        print(cnt)
